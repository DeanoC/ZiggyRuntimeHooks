...
test "run_engine: integration lifecycle handles abort resume and completion" {
    const allocator = std.testing.allocator;

    const dir = try std.fmt.allocPrint(allocator, ".tmp-run-engine-lifecycle-{d}", .{std.time.nanoTimestamp()});
    defer allocator.free(dir);
    defer std.fs.cwd().deleteTree(dir) catch {};
    try std.fs.cwd().makePath(dir);
    ...
    var started = try engine.start("input-a");
    defer started.deinit(allocator);

    var first_step = try engine.beginStep(started.run_id, null);
    defer first_step.deinit(allocator);
    try std.testing.expectEqualStrings("input-a", first_step.input);

    var waiting = try engine.completeStep(started.run_id, "need more", true, false);
    defer waiting.deinit(allocator);
    try std.testing.expectEqual(RunState.waiting_for_user, waiting.state);

    var queued = try engine.enqueueInput(started.run_id, "input-b");
    defer queued.deinit(allocator);

    var second_step = try engine.beginStep(started.run_id, null);
    defer second_step.deinit(allocator);
    try std.testing.expectEqualStrings("input-b", second_step.input);

    var aborted = try engine.abortStep(started.run_id, "manual stop", true);
    defer aborted.deinit(allocator);
    try std.testing.expectEqual(RunState.paused, aborted.state);

    var resumed_step = try engine.beginResumedStep(started.run_id, null);
    defer resumed_step.deinit(allocator);
    try std.testing.expectEqualStrings("input-b", resumed_step.input);

    var completed = try engine.completeStep(started.run_id, "done", false, true);
    defer completed.deinit(allocator);
    try std.testing.expectEqual(RunState.completed, completed.state);
    try std.testing.expectEqual(@as(u64, 3), completed.step_count);

    const events = try engine.listEvents(allocator, started.run_id, 64);
    defer deinitEvents(allocator, events);

    var saw_abort = false;
    var saw_complete = false;
    for (events) |event| {
        if (std.mem.eql(u8, event.event_type, "run.step_aborted")) saw_abort = true;
        if (std.mem.eql(u8, event.event_type, "run.step_completed")) saw_complete = true;
    }
